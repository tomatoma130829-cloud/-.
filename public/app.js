...
function renderOnline() {
  el.onlineList.innerHTML = state.onlineUsers.map((name) => `<li>${name} 🟢 オンライン</li>`).join('');
  if (state.user) {
...
function renderUser() {
  ...
  el.stampSelect.innerHTML = state.user.stamps.map((stamp) => `<option value="${stamp}">${stamp}</option>`).join('');
  renderOnline();
}
...
async function loadAdminDashboard() {
  ...
  el.adminUsers.innerHTML = data.users.map((u) => `<li>${u.username} / ${u.points}pt / ${u.online ? '🟢' : '⚪'}</li>`).join('');
  el.notificationList.innerHTML = data.notifications.slice(0, 20).map((n) => `<li>[${n.type}] ${n.message}</li>`).join('');
  el.logList.innerHTML = data.logs.slice(0, 20).map((l) => `<li>${l.timestamp} - ${l.message}</li>`).join('');
}
...
function initGame(game) {
  if (game === 'battle') {
    let hpA = 100;
    let hpB = 100;
    el.gameArea.innerHTML = `<h4>バトル対戦 (2人/CPU)</h4><p>あなたHP: <span id="hpA">${hpA}</span> / 相手HP: <span id="hpB">${hpB}</span></p><button id="attackBtn">攻撃</button>`;
  ...
  if (game === 'tag') {
    el.gameArea.innerHTML = `<h4>鬼ごっこ対戦 (2人/CPU)</h4><p>鬼から逃げ切る確率ゲーム。</p><button id="runBtn">逃げる</button><p id="tagResult"></p>`;
  ...
  if (game === 'shooting') {
    let enemyHp = 80;
    el.gameArea.innerHTML = `<h4>シューティング対戦</h4><p>敵HP: <span id="enemyHp">${enemyHp}</span></p><button id="shootBtn">弾発射</button>`;
  ...
  if (game === 'future') {
    el.gameArea.innerHTML = `<h4>自由追加枠</h4><p>ここに新ゲームを後から追加できます。</p>`;
  }
}
...
document.getElementById('registerBtn').onclick = async () => {
  try {
    await api('/api/register', 'POST', { username: el.username.value.trim(), password: el.password.value });
...
function enterApp() {
  ...
  socket.emit('auth:session', { token: state.token });
  socket.emit('auth:user-online', { username: state.username, role: state.role, token: state.token });
}

document.getElementById('loginBtn').onclick = async () => {
  try {
    const data = await api('/api/login', 'POST', { username: el.username.value.trim(), password: el.password.value });
...
document.getElementById('adminLoginBtn').onclick = async () => {
  try {
    const data = await api('/api/login', 'POST', { adminId: el.adminId.value.trim(), password: el.adminPassword.value });
    state.username = data.username;
    state.role = 'admin';
    state.token = data.token;
    enterApp();
...
document.getElementById('grantBtn').onclick = async () => {
  try {
    await api('/api/admin/grant', 'POST', { username: el.grantUser.value.trim(), points: Number(el.grantPoints.value || 0), item: el.grantItem.value.trim() });
    await loadAdminDashboard();
  } catch (error) {
...
socket.on('chat:message', ({ username, message }) => addChatLine(`${username}: ${message}`));
socket.on('chat:stamp', ({ username, stamp }) => addChatLine(`${username}: [STAMP] ${stamp}`));
...
socket.on('force:logout', async ({ reason }) => {
