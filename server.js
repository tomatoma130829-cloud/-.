...
function createSession(username, role) {
  ...
  sessions.set(token, { username, role, createdAt: new Date().toISOString() });
  return token;
}
...
app.post('/api/login', (req, res) => {
  ...
  if (adminId) {
    if (adminId === ADMIN_ID && password === ADMIN_PASSWORD) {
      ...
      const token = createSession(ADMIN_ID, 'admin');
      return res.json({ success: true, role: 'admin', username: ADMIN_ID, token });
    }
  ...
  const token = createSession(user.username, 'user');
  return res.json({ success: true, role: 'user', username: user.username, token, user: userPublicInfo(user, getOnlineData().onlineUsers) });
});
...
app.get('/api/state/:username', requireRole('user'), (req, res) => {
...
app.post('/api/gacha', requireRole('user'), (req, res) => {
...
app.post('/api/reward', requireRole('user'), (req, res) => {
...
app.post('/api/admin/grant', requireRole('admin'), (req, res) => {
...
app.post('/api/admin/force-logout', requireRole('admin'), (req, res) => {
  const { username } = req.body;
...
io.on('connection', (socket) => {
  ...
  socket.on('auth:user-online', ({ username, role, token }) => {
    const session = sessions.get(token);
    if (!session || session.username !== username || session.role !== role) return;
